  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Generated commercial bids</div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Under customer review</div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          ...
          <div className="text-xl sm:text-2xl font-black text-emerald-700 dark:text-emerald-400 mt-1 font-mono">
            {acceptedCount}
          </div>
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Approved & contracted</div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">In client consideration</div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-3.5 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Total closed revenue</div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2.5 text-xs">
          ...
          <div className="relative sm:col-span-2">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 rounded-lg text-xs font-medium focus:ring-2 focus:ring-[#5B4DB7] focus:outline-none"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-2 text-xs text-slate-700 dark:text-slate-200 font-medium focus:ring-1 focus:ring-[#5B4DB7]"
            >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs overflow-hidden">
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            <thead className="bg-slate-50/90 dark:bg-slate-950/60 border-b border-slate-200 dark:border-slate-800 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {filteredProposals.length > 0 ? (
                filteredProposals.map((proposal) => (
                  ...
                  >
                    {/* Proposal Code */}
                    <td className="py-3.5 px-4 font-mono font-bold text-[#5B4DB7] dark:text-purple-300 group-hover:underline">
                    ...
                    {/* Opportunity */}
                    <td className="py-3.5 px-3">
                      <span className="font-medium text-slate-800 dark:text-slate-200 line-clamp-1 max-w-[170px]" title={proposal.opportunityName}>
                    ...
                    >
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          title="Send / Resend"
                          className="p-1.5 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 rounded-md transition-colors cursor-pointer"
                        >
                ...
                <tr>
                  <td colSpan={10} className="py-12 text-center text-xs text-slate-400 dark:text-slate-500">
          ...
          {filteredProposals.length > 0 ? (
            filteredProposals.map((proposal) => (
              <div
                key={proposal.id}
                onClick={() => setSelectedProposal(proposal)}
                className="p-4 space-y-2.5 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors cursor-pointer"
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="font-mono text-xs font-bold text-[#5B4DB7] dark:text-purple-300">
                      {proposal.proposalCode}
                    </span>
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white leading-tight">
                      {proposal.companyName}
                    </h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{proposal.opportunityName}</p>
                  </div>
                  <ProposalStatusBadge status={proposal.status} size="xs" />
                </div>

                <div className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-950/60 p-2.5 rounded-lg border border-slate-100 dark:border-slate-800">
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <span className="font-semibold text-slate-800 dark:text-slate-200">{proposal.validUntil}</span>
                  </div>
                </div>
                ...
                >
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 rounded-md text-xs font-semibold flex items-center gap-1 cursor-pointer"
                  >
            ...
            <div className="p-8 text-center text-xs text-slate-400 dark:text-slate-500">No proposals found.</div>
          )}
        </div>
      </div>
