...
export const CallFiltersDrawer: React.FC<CallFiltersDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* Drawer Panel */}
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 h-full shadow-2xl flex flex-col z-10 animate-in slide-in-from-right duration-200">
        {/* Header */}
        <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h3 className="font-bold text-slate-900 dark:text-white text-base">Filter Calls</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Refine call logs by parameters</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 text-xs">
          {/* Date Filter */}
          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Date Range</label>
            <select
          ...
          {/* Call Type */}
          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Call Direction / Type</label>
            <select
          ...
          {/* Status */}
          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Call Status</label>
            <select
          ...
          {/* Result */}
          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Call Outcome / Result</label>
            <select
          ...
          {/* Service */}
          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">TechnoKraft Service</label>
            <select
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 flex items-center justify-between gap-3">
          <button
            ...
            className="px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-semibold hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center gap-1.5 min-h-[44px] cursor-pointer transition-colors"
          >
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex-1 px-4 py-2.5 rounded-lg bg-[#5B4DB7] hover:bg-[#4E41A2] text-white font-semibold flex items-center justify-center gap-1.5 shadow-2xs min-h-[44px] cursor-pointer transition-colors"
          >
