...
export const CallCard: React.FC<CallCardProps> = ({
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-900 rounded-xl border transition-all p-4 space-y-3.5 shadow-2xs ${
        isSelected
    ...
    >
      ...
      <div className="space-y-1">
        <div className="flex items-start justify-between gap-2">
          <h4
            onClick={() => onView(call)}
            className="text-sm font-bold text-slate-900 dark:text-white hover:text-[#5B4DB7] dark:hover:text-purple-400 cursor-pointer flex items-center gap-1.5 transition-colors"
          >
        ...
        <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300 pl-5.5">
        ...
        <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 pl-5.5">
      ...
      <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2">
          ...
          {call.duration && call.duration !== '0m 00s' && (
            <span className="inline-flex items-center gap-1 font-mono text-[11px] text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200/50 dark:border-slate-700">
      ...
      <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 pt-1">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-purple-100 dark:bg-purple-950/80 text-[#5B4DB7] dark:text-purple-300 font-bold text-[10px] flex items-center justify-center">
            {call.employeeAvatar}
          </div>
          <span className="font-medium text-slate-700 dark:text-slate-300">{call.employeeName}</span>
        </div>

        {call.nextFollowUp && (
          <span className="text-[11px] text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/50 px-2 py-0.5 rounded-full border border-indigo-100 dark:border-indigo-800">
      ...
      <div className="pt-2 flex items-center gap-2">
        <button
          type="button"
          onClick={() => onView(call)}
          className="flex-1 min-h-[44px] px-3 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
        >
          <span>View</span>
        </button>
        ...
        <button
          ...
          className="flex-1 min-h-[44px] px-3 py-2 bg-[#5B4DB7] hover:bg-[#4E41A2] text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors shadow-2xs cursor-pointer"
        >
          <span>Follow-up</span>
        </button>
        ...
        {isOpportunityEligible && onCreateOpportunity && (
          <button
            ...
            className="min-h-[44px] px-3 py-2 bg-amber-50 dark:bg-amber-950/40 hover:bg-amber-100 dark:hover:bg-amber-900/40 text-amber-900 dark:text-amber-200 border border-amber-200 dark:border-amber-800 rounded-lg text-xs font-semibold flex items-center justify-center gap-1 cursor-pointer transition-colors"
