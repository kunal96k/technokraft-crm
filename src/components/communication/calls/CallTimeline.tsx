  ...
  if (calls.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-slate-200/90 p-8 text-center text-slate-500">
        ...
        <p className="text-sm font-semibold text-slate-700">No calls in timeline</p>
        <p className="text-xs text-slate-400 mt-0.5">Try changing the filters above</p>
      </div>
    );
  }
          ...
          <div key={dateStr} className="space-y-3">
            ...
            <div className="relative pl-6 space-y-4 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
              {dateCalls.map((call) => {
                ...
                if (call.status === 'completed') dotColor = 'bg-emerald-500 ring-emerald-100';
                if (call.status === 'missed') dotColor = 'bg-rose-500 ring-rose-100';
                if (call.status === 'scheduled') dotColor = 'bg-indigo-500 ring-indigo-100';

                return (
                  <div key={call.id} className="relative group">
                    {/* Node Dot */}
                    <div
                      className={`absolute -left-6 top-3 w-4 h-4 rounded-full border-2 border-white ring-4 shadow-2xs ${dotColor}`}
                    />

                    {/* Timeline Item Card */}
                    <div className="bg-white rounded-xl border border-slate-200/90 p-4 shadow-2xs hover:border-[#5B4DB7]/40 transition-all">
                      ...
                      <div className="pt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            <h4
                              onClick={() => onView(call)}
                              className="text-xs font-bold text-slate-900 hover:text-[#5B4DB7] cursor-pointer"
                            >
                              {call.companyName}
                            </h4>
                          ...
                          <div className="flex items-center gap-1.5 text-xs text-slate-600 pl-5">
                        ...
                        <div className="space-y-1 sm:text-right flex flex-col justify-between">
                          <div className="flex sm:justify-end items-center gap-1.5 text-xs text-slate-600">
                            <span className="text-slate-400">Called by:</span>
                            <div className="w-5 h-5 rounded-full bg-purple-100 text-[#5B4DB7] font-bold text-[10px] flex items-center justify-center">
                              {call.employeeAvatar}
                            </div>
                            <span className="font-semibold text-slate-800">{call.employeeName}</span>
                          </div>

                          {call.nextFollowUp && (
                            <div className="text-xs text-indigo-700 bg-indigo-50/80 px-2.5 py-1 rounded-lg border border-indigo-100/80 sm:self-end">
                      ...
                      {/* Notes snippet */}
                      {call.notes && (
                        <div className="mt-3 p-2.5 rounded-lg bg-slate-50/90 border border-slate-200/70 text-xs text-slate-700 flex items-start gap-2">
                      ...
                      <div className="mt-3 pt-2 border-t border-slate-100 flex items-center justify-end gap-2">
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 text-xs text-slate-600 hover:text-slate-900 rounded hover:bg-slate-100"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 text-xs text-[#5B4DB7] font-medium hover:bg-purple-50 rounded"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => onView(call)}
                          className="inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold text-white bg-[#5B4DB7] hover:bg-[#4E41A2] rounded-lg shadow-2xs"
                        >
