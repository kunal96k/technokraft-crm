...
export const EmailListItem: React.FC<EmailListItemProps> = ({
  ...
  return (
    <div
      ...
      className={`group relative flex items-start sm:items-center justify-between p-3 sm:p-3.5 border-b border-slate-100 cursor-pointer transition-all ${
        isActive
    ...
    >
      <div className="flex items-start sm:items-center gap-3 min-w-0 flex-1 pr-3">
        ...
        >
          <input
            ...
            checked={isSelected}
            onChange={() => {}}
            className="w-4 h-4 text-[#5B4DB7] rounded border-slate-300 focus:ring-[#5B4DB7]/40 cursor-pointer"
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <div className="flex items-center gap-2 text-[11px] text-slate-400 sm:hidden pt-0.5">
      ...
      <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 sm:gap-4 flex-shrink-0">
        ...
        <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-[#5B4DB7] hidden sm:block transition-colors" />
      </div>
    </div>
  );
};
