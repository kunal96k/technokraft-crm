      ...
      <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-4 shadow-2xs space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <CalendarIcon className="w-5 h-5 text-[#5B4DB7]" />
          ...
          <div className="flex items-center gap-1 text-slate-500 text-xs">
            ...
            <span className="px-2 font-medium">Sep 2026</span>
            <button
              type="button"
              className="p-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors"
              title="Next Month"
        ...
        {/* Days of week header */}
        <div className="grid grid-cols-7 gap-1 text-center text-xs font-semibold text-slate-500 py-1 border-b border-slate-100">
            ...
            return (
              <button
                ...
                className={`h-16 sm:h-20 p-1.5 rounded-lg text-left flex flex-col justify-between border transition-all ${
                  isSelected
                    ? 'border-[#5B4DB7] bg-purple-50/60 ring-2 ring-[#5B4DB7]/30 shadow-xs'
                    : isToday
              ...
              >
                ...
                <div className="space-y-0.5 overflow-hidden">
                  {items.slice(0, 2).map((it) => (
                    <div
                      key={it.id}
                      className="text-[9px] truncate px-1 py-0.2 rounded bg-slate-100 text-slate-700 font-medium"
                    >
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
          ...
          {selectedDate === '2026-09-07' && (
            <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-purple-100 text-[#5B4DB7]">
              Today
            </span>
          )}
        </div>
        ...
        {selectedDayItems.length === 0 ? (
          <div className="p-8 text-center text-xs text-slate-400 space-y-1">
            ...
            <p className="font-medium text-slate-600">No scheduled follow-ups</p>
          ...
          <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
            {selectedDayItems.map((item) => (
              <div
                key={item.id}
                className="p-3 rounded-lg border border-slate-200 bg-slate-50/50 hover:bg-slate-50 hover:border-[#5B4DB7]/40 transition-all space-y-2"
              >
                <div className="flex items-center justify-between gap-1">
                  <span className="font-mono text-xs font-bold text-[#5B4DB7] bg-purple-50 px-1.5 py-0.5 rounded">
                    {item.time}
                  </span>
                ...
                <div>
                  <Link
                    ...
                    className="font-bold text-xs text-slate-900 hover:text-[#5B4DB7] flex items-center gap-1"
                  >
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1.5 border-t border-slate-200/60">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <button
                      ...
                      className="text-[11px] font-medium text-[#5B4DB7] hover:underline"
                    >
                      View
                    </button>
                  </div>
