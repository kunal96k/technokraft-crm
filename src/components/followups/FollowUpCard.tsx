  ...
  return (
    <div
      ...
      className={`p-4 rounded-xl border transition-all duration-150 relative ${
        isSelected
          ? 'border-[#5B4DB7] bg-purple-50/20'
          : isOverdue
          ? 'border-red-200/90 bg-red-50/10 hover:border-red-300'
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative" ref={menuRef}>
            ...
            {showMenu && (
              <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-slate-200 rounded-lg shadow-lg z-20 py-1 text-xs">
                <button
                  type="button"
                  onClick={() => {
                    setShowMenu(false);
                ...
                {followUp.status !== 'COMPLETED' && (
                  <>
                    <button
                      ...
                      className="w-full text-left px-3 py-1.5 hover:bg-slate-50 text-emerald-700 font-medium"
                    >
                      Complete Follow-up
                    </button>
                    <button
                      ...
                      className="w-full text-left px-3 py-1.5 hover:bg-slate-50 text-purple-700"
                    >
                      Reschedule
                    </button>
                  </>
                ...
                {followUp.status !== 'CANCELLED' && (
                  <button
                    ...
                    className="w-full text-left px-3 py-1.5 hover:bg-red-50 text-red-600 border-t border-slate-100 mt-1"
                  >
                    Cancel Follow-up
                  </button>
                )}
      ...
      <div className="mt-3">
        <div className="flex items-center gap-1.5 text-xs text-slate-500 font-mono">
          ...
          <span>•</span>
          <span className="text-slate-600 truncate">{followUp.service}</span>
        </div>
        <Link
          to={`/leads/${followUp.leadId}`}
          className="text-sm font-semibold text-slate-900 hover:text-[#5B4DB7] flex items-center gap-1 mt-0.5"
        >
      ...
      {/* Purpose */}
      <p className="mt-2 text-xs text-slate-700 font-medium bg-slate-50 p-2 rounded-lg border border-slate-100 line-clamp-2">
        {followUp.purpose}
      </p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600 border-t border-slate-100 pt-2.5">
        ...
        <div className="flex items-center justify-end gap-1.5">
          ...
          <span className="truncate text-slate-600 text-[11px]">{followUp.assignedTo}</span>
        </div>
      </div>
      ...
      {followUp.status !== 'COMPLETED' && followUp.status !== 'CANCELLED' && (
        <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 py-1.5 px-3 rounded-lg text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition-colors"
          >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 py-1.5 px-3 rounded-lg text-xs font-medium text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition-colors"
          >
