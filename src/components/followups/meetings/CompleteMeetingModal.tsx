        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
            />
          </div>

          {/* Next Action Box */}
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
            <label className="flex items-center gap-2 cursor-pointer font-semibold text-slate-800 dark:text-slate-200 mb-2">
              <input
                type="checkbox"
                checked={createNextFollowUp}
                onChange={(e) => setCreateNextFollowUp(e.target.checked)}
                className="w-4 h-4 rounded text-[#5B4DB7] focus:ring-[#5B4DB7]/40 border-slate-300 dark:border-slate-600"
              />
              <span>Create follow-up task automatically from this meeting</span>
            </label>

            {createNextFollowUp && (
              <div className="p-3 bg-purple-50/50 dark:bg-purple-950/20 rounded-xl border border-purple-200/80 dark:border-purple-900/40 space-y-3 animate-in fade-in duration-150">
