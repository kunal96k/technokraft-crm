      ...
      >
        ...
        <div className="p-6 space-y-5 text-xs">
          ...
          <div>
            ...
            {followUp.service && (
              <p className="text-purple-700 dark:text-purple-400 font-medium text-xs mt-0.5">
          ...
          <div className="p-3.5 bg-slate-50 dark:bg-slate-950/60 rounded-xl border border-slate-200/80 dark:border-slate-800 space-y-1">
            ...
            {followUp.notes && (
              <p className="text-xs text-slate-500 dark:text-slate-400 italic pt-1 border-t border-slate-200/60 dark:border-slate-800 mt-1">
          ...
          {/* Timing & Reminders */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-3 bg-purple-50/40 dark:bg-purple-950/20 rounded-xl border border-purple-100 dark:border-purple-900/40">
            <div>
            ...
            <div>
              <span className="text-[10px] text-slate-400 dark:text-slate-500 block font-medium">Reminder</span>
              <span className="font-medium text-slate-700 dark:text-slate-300 text-xs block mt-0.5">
          ...
          <div className="space-y-2">
            ...
            <div className="p-3 bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-700 dark:text-slate-300">
              <div className="flex items-center gap-2">
                <User className="w-4 h-4 text-slate-400 dark:text-slate-500" />
                <div>
                  <span className="font-semibold block text-slate-900 dark:text-white">{followUp.contactName}</span>
          ...
          {/* Owner Details */}
          <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-950/60 rounded-xl border border-slate-200/80 dark:border-slate-800">
            <div className="flex items-center gap-2">
              <span className="w-8 h-8 rounded-full bg-purple-100 dark:bg-purple-900/40 text-[#5B4DB7] dark:text-purple-300 text-xs font-bold flex items-center justify-center">
            ...
            <Link
              ...
              className="flex items-center gap-1 text-xs font-semibold text-[#5B4DB7] dark:text-purple-400 hover:underline"
            >
          ...
          {followUp.status === 'COMPLETED' && (
            <div className="p-3.5 bg-emerald-50 dark:bg-emerald-950/30 rounded-xl border border-emerald-200 dark:border-emerald-900/50 text-emerald-900 dark:text-emerald-300 space-y-1">
          ...
          {followUp.status === 'RESCHEDULED' && (
            <div className="p-3.5 bg-purple-50 dark:bg-purple-950/30 rounded-xl border border-purple-200 dark:border-purple-900/50 text-purple-900 dark:text-purple-300 space-y-1">
              <div className="flex items-center gap-1.5 font-bold text-xs">
                <RotateCcw className="w-4 h-4 text-purple-600 dark:text-purple-400" />
                <span>Rescheduled</span>
              </div>
              {followUp.rescheduleReason && (
                <p className="text-xs text-purple-800/90 dark:text-purple-300/90 mt-1">
            ...
            {followUp.status !== 'COMPLETED' && (
              <>
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg transition-colors"
                >
                  Reschedule
                </button>
                <button
